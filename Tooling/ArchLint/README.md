...
- `Sources/ArchLintCore/` — 규칙 12개(파일 단위 9 + 모듈 단위 3) + 스캔/파싱 로직(라이브러리)
...
| ⑬ Feature Factory 배타성 | `feature-exclusivity` | **error** | `Projects/Feature/<Module>` (모듈 단위) | ⑫를 Feature로 옮김 — Feature 모듈의 top-level public은 `*FeatureFactory`만(⑫의 `*DataFactory`와 대칭 · View·VM·상태는 internal, Factory는 `some View` 반환). Data와 달리 **조립 seam 예외**: `Sources/Navigation/`의 **계약 타입(typealias·protocol + `*Route` enum, #253)만** 허용(App이 Feature 간 직접 의존 없이 콘텐츠 주입/화면 전환 의도 수신 — 같은 폴더라도 구체 View/VM·Route 아닌 enum은 위반). 판정 로직은 ⑫와 `publicTopLevelOffense` 공유(접미사만 `Factory`). `extension`은 외부 public 타입에 public 멤버를 붙일 때만 노출로 본다(로컬 internal 타입 확장은 제외). |
| ⑭ Feature 라우트 콜백 | `feature-route-callback` | warning | `Projects/{Feature}/**/Sources` | Feature **public 함수**(사실상 Factory 진입점 — ⑬이 그 외 public을 막음)의 `on…Tapped:`/`on…Selected:` 클로저 파라미터 금지 — 화면 전환 의도는 모듈 `*Route` enum + `onRoute:` 하나로(#253, 정본 `NovelDetailFeature/Sources/Navigation/NovelDetailRoute.swift`). 네이밍 관행 기반 프록시라 warning(비-라우팅 콜백 `onSubmitted`/`onSaved` 등 과거형 네이밍은 정상적으로 안 걸림). internal 서브뷰의 로컬 콜백은 대상 아님. |
