...
| ⑬ Feature Factory 배타성 | `feature-exclusivity` | **error** | `Projects/Feature/<Module>` (모듈 단위) | ⑫를 Feature로 옮김 — Feature 모듈의 top-level public은 `*FeatureFactory`만(⑫의 `*DataFactory`와 대칭 · View·VM·상태는 internal, Factory는 `some View` 반환). Data와 달리 **조립 seam 예외**: `Sources/Navigation/`의 **계약 타입(typealias·protocol)만** 허용(App이 Feature 간 직접 의존 없이 콘텐츠 주입 — 같은 폴더라도 구체 View/VM은 위반). 판정 로직은 ⑫와 `publicTopLevelOffense` 공유(접미사만 `Factory`). `extension`은 외부 public 타입에 public 멤버를 붙일 때만 노출로 본다(로컬 internal 타입 확장은 제외). |
