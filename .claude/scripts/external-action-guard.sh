...
#   - new-issue.sh preflight(읽기·안전) / 그 외 명령 → 빈 출력 → 기본 권한 흐름(가드 개입 없음).
#
# 설계 메모: 직접 gh 명령(gh pr create 등)은 일부러 가드하지 않는다.
#   외부작업은 가드된 스크립트를 통해서만 한다는 규율을 전제로, 그 스크립트 실행만 게이트한다.
#   향후 외부작업 스크립트가 늘면 아래 패턴(RUN_RE)에 추가한다.
set -euo pipefail

cmd="$(cat | jq -r '.tool_input.command // ""')"

# 외부작업 스크립트의 비가역 서브커맨드 호출
RUN_RE='(^|[^[:alnum:]_-])new-issue\.sh[[:space:]]+run([[:space:]]|$)'

if printf '%s' "$cmd" | grep -qE "$RUN_RE"; then
