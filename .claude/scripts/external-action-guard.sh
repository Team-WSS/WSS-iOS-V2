...
#   - ready-merge.sh push / rebase.sh push (--force-with-lease 강제 푸시) → permissionDecision=ask → 사용자 승인 프롬프트.
...
FORCE_PUSH_RE='(^|[^[:alnum:]_-])(ready-merge|rebase)\.sh[[:space:]]+push([[:space:]]|$)'

if printf '%s' "$cmd" | grep -qE "$FORCE_PUSH_RE"; then
  printf '%s' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"ask","permissionDecisionReason":"외부로 나가는 비가역 작업입니다(--force-with-lease 강제 푸시로 원격 브랜치를 덮어씁니다). 승인이 필요합니다."}}'
fi
exit 0
