...
>
> ⚠️ **구현은 메인이 직접, 리뷰만 위임.** 리뷰는 격리 컨텍스트가 유리해 `wss-pr-reviewer`(화면 비중이 크면 `wss-feature-reviewer`도 — 1단계 "리뷰어 편성 판정") subagent **또는 사용자가 택한 Codex CLI**(1단계 "리뷰 수단 확인")에 위임하고, 받은 리포트로 **수정은 메인이** 한다. 문서 갱신 판정·편집, PR 본문 작성도 메인이 직접 한다.
>
...
- **리뷰 수단 확인 (1라운드 전 1회 — 사용자에게 묻는다)**: 리뷰를 **wss 서브에이전트로 할지, Codex CLI로 할지, 둘 다 병렬로 할지** 사용자에게 물어 확정한다. **기본값을 두지 않는다** — 팀원마다 Codex CLI 설치 여부가 다르다(없는 사람에게 Codex를 전제하면 그 자리에서 막힌다). 서브에이전트는 아래 편성 판정대로, Codex는 `codex exec --sandbox read-only "<프롬프트>" < /dev/null > <출력파일> 2>&1`을 백그라운드로(`exec review` 서브커맨드는 `--base`와 커스텀 프롬프트를 같이 못 받으니 일반 `exec`에 레포 규약·`git diff origin/develop...HEAD` 범위·출력 형식(심각도+파일:라인+실패 시나리오)을 직접 담는다. 출력 파일의 `codex` 줄 이후가 리포트 — exit 0이어도 `ERROR:`만 남을 수 있으니 눈으로 확인). 병렬이면 두 리포트가 겹치는 곳은 신뢰도가 오른 것, 엇갈리면 메인이 정본 판정.
...
- **문서 감사 패스 (diff에서 문서 비중이 크면 — 리뷰 수단과 무관하게 별도로, 같은 라운드에 병렬)**: 코드 리뷰어(서브에이전트든 Codex든)는 `*.md`를 보지 않아 **"지적 없음"이 나와도 문서는 미검사**다. 계약서·`docs/*`·`CLAUDE.md`가 diff의 큰 몫이면 **문서 감사 전용 프롬프트**를 따로 돌린다(서브에이전트면 `general-purpose` Agent, Codex면 두 번째 `codex exec`). 점검 축: 요약 배지↔본문/문서 간 desync · 상대경로 링크·내부 앵커 무결성 · 문서↔코드 `파일:라인` 인용 정확성(코드에 주석만 더해도 밀린다) · 판정 근거↔결론 일관성 · 수치 자기모순 · 에이전트 정의↔실제 산출물 형식. "짧은 문서는 결함이 아니다"(레포 철학)를 프롬프트에 명시해 문체·분량 지적을 막는다. 결과는 코드 리뷰와 같은 🔴/🟡/🔵 기준으로 수렴 루프에 합류시킨다(#222에서 코드 리뷰 0건·문서 감사 🔴2 🟡6이 나온 실측 근거).
- **루프 (최대 3라운드)**:
  1. **리뷰** — 확정한 수단으로 호출한다: 서브에이전트면 `Agent` 도구로 `subagent_type: wss-pr-reviewer`(+ 위 판정 시 `wss-feature-reviewer`), Codex면 `codex exec`(위 "리뷰 수단 확인"). 문서 감사 패스가 편성됐으면 같은 라운드에 함께 띄운다. Codex는 세션이 안 이어지므로 **이전 라운드에서 기각한 지적은 다음 프롬프트에 "다시 올리지 마라"로 못 박는다**.
...
- **어투는 해요체("~해요"/"~돼요")로 통일한다 — 합쇼체("~합니다") 금지.** 가독성도 신경 쓴다: 섹션 안에서도 소제목(볼드)으로 관심사를 나누고, 문장은 짧게 끊는다. 커밋 로그를 나열하는 게 아니라 리뷰어가 읽고 바로 이해하게 쓴다.
  - **한 문단·한 불릿 안에 문장이 두 개 이상 이어져 길어지면 줄바꿈한다** — 불릿이면 문장 단위로 별도 불릿으로 쪼개고, 일반 문단(Summary 등)이면 문장 사이에 빈 줄을 넣어 나눈다. 한 줄에 여러 문장을 욱여넣지 않는다.
