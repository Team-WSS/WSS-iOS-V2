---
name: make-PR
description: WSS-iOS-V2 프로젝트에서, 작업을 마치고 PR을 올릴 때 사용한다. ① wss-pr-reviewer(화면 비중이 크면 wss-feature-reviewer 병렬)로 변경 전체를 통합 리뷰→수정을 Blocker·Warning이 0이 될 때까지 수렴 → ② 이번 변경과 관련된 CLAUDE.md/docs 갱신 필요를 메인이 직접 판정해 필요한 것만 방향성과 함께 제시·반영 → ③ 문서 변경 커밋 → ④ 프로젝트 PR 템플릿대로 본문 작성·사용자 검토 → ⑤ 승인 후 gh로 GitHub PR 생성, 까지 단계별 게이트로 진행한다. "작업 마무리하자", "PR 올리자", 또는 "/make-PR" 같은 요청에 트리거. ⚠️ push·PR 생성은 외부로 나가는 비가역 작업 → 승인 후에만 실행한다. (머지 직전 rebase·force push·merge는 이 스킬 범위 밖.)
metadata:
...
>
> ⚠️ **구현은 메인이 직접, 리뷰만 위임.** 리뷰는 격리 컨텍스트가 유리해 `wss-pr-reviewer`(화면 비중이 크면 `wss-feature-reviewer`도 — 1단계 "리뷰어 편성 판정") subagent에 위임하고, 받은 리포트로 **수정은 메인이** 한다. 문서 갱신 판정·편집, PR 본문 작성도 메인이 직접 한다.
>
...
### 1. 통합 코드 리뷰 — 리뷰어 위임, 수정은 메인 (수렴 루프)
...
- **리뷰어 편성 판정 (호출 전, 매 라운드 아님 — 1라운드에 한 번)**: 이번 diff와 브랜치 이슈를 보고 **화면 작업 비중**을 판정해, 크면 `wss-feature-reviewer`를 `wss-pr-reviewer`와 **같은 라운드에 병렬 호출**한다(순차 ❌ — 동시에 띄우면 벽시계 비용이 없다). 판정 신호:
  - 신규 Feature 모듈·새 화면/시트 추가 → 거의 항상 병렬
  - `Projects/Feature/` 변경이 이번 diff에서 파일 수·라인 수로 큰 몫
  - 브랜치·이슈 Type이 `Feat`·`Design`이고 목적 자체가 화면 구현

  국소 변경(상수·문구·여백 조정)뿐이면 생략한다. **어느 쪽이든 편성 결과를 한 줄로 보고**한다("Feature 비중 낮음 → pr-reviewer 단독"). 두 리포트가 겹치면 신뢰도가 오른 것으로 보고, 심각도가 엇갈리면 **메인이 정본으로 판정**한다.
- **루프 (최대 3라운드)**:
  1. **리뷰** — `Agent` 도구로 `subagent_type: wss-pr-reviewer`(+ 위 판정 시 `wss-feature-reviewer`)를 호출한다.
...
- `.github/pull_request_template.md`를 **읽어** 그 섹션 구조 그대로 채운다(하드코딩 금지). 현재 6섹션:
  - `💡 Issue` → **`- closed #<번호>`**(하이픈 접두 필수 — 템플릿 예시 주석 `<!-- closed #1 -->`엔 하이픈이 없지만, 실제로 채울 땐 항상 리스트 아이템 형태로 쓴다. 0단계에서 추출한 번호)
  - `💭 Summary` → 작업 한 줄 소개
...
- ⚠️ **사용자 승인 전까지 어떤 외부 동작(push·PR 생성)도 하지 않는다.** 승인 시:
  ...
  2. 4단계에서 승인된 **본문을 스크래치패드 임시 파일에 기록**(멀티라인 인자 회피)한 뒤:
     ...
     gh pr create --base develop --head <branch> \
       ...
       --assignee @me
     ```
  - 계정/소유자/레포는 하드코딩하지 않는다 — 현재 레포로 자동 감지되고, **담당자는 `@me`(실행한 본인)로 자동 지정**한다(`new-issue`의 이슈 담당자 규칙과 동일).
...
- **리뷰만 위임, 나머지는 메인.** 완성 = 코드 작성이 아니라 **리뷰 수렴 + 문서 동기화**: 편성한 리뷰어 **전원**의 🔴+🟡=0 + 관련 문서가 변경을 반영해야 PR을 올린다.
