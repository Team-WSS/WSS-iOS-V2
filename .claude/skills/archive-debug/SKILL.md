---
name: archive-debug
description: WSS-iOS-V2에서 fastlane으로 Debug 스킴(`WSS-iOS-DEBUG`, `.env`의 `APP_IDENTIFIER_DEBUG`)을 아카이브해 TestFlight 내부 테스터용으로 올릴 때 사용한다(`fastlane/Fastfile`의 `debug_beta` lane 호출 — 서명은 match, 업로드는 App Store Connect API 키로 인증). ① 사전 점검(fastlane/.env·Bundler·MATCH_PASSWORD·ASC .p8 존재) → ② 무엇을 아카이브·업로드할지 보여주고 승인받음 → ③ 승인 후 `bundle exec fastlane debug_beta` 백그라운드 실행 → ④ 결과(빌드 번호·TestFlight 처리 상태 또는 실패 원인)를 있는 그대로 보고, 까지 한다. "debug 아카이브해줘", "디버그 TestFlight 올려줘", "debug 베타 배포", 또는 "/archive-debug" 같은 요청에 트리거. ⚠️ 빌드 서명·TestFlight 업로드는 외부로 나가는 비가역 작업 → 승인 후에만 실행한다. Release 스킴은 `archive-release` 스킬이 담당 — 이 스킬은 Debug 전용.
metadata:
  short-description: fastlane debug_beta로 Debug 스킴 아카이브 + TestFlight 업로드
---

# Archive Debug — fastlane Debug 아카이브 + TestFlight 업로드 (WSS-iOS-V2)

`fastlane/Fastfile`의 `debug_beta` lane을 호출해 **Debug 스킴만** 아카이브 → TestFlight 내부 테스터용
업로드까지 한 번에 한다. 인자 없음(대상이 Debug 하나로 고정).

- lane: `debug_beta`
- scheme: `WSS-iOS-DEBUG`
- app identifier: `.env`의 `APP_IDENTIFIER_DEBUG` 값

> ⚠️ **서명·업로드는 외부로 나가는 비가역 작업이다** — 빌드가 실제로 Apple 서버에 업로드되고 내부
> 테스터에게 노출될 수 있다. **3단계 승인 전에는 실행하지 않는다.**
>
> Release 스킴을 아카이브하려면 이 스킬이 아니라 **`archive-release`** 를 쓴다(서로 다른 스킴이라
> 헷갈리지 않게 스킬 자체를 분리했다). App Store 심사 제출(`release` lane, `deliver`)은 둘 다 범위
> 밖 — 어느 아카이브 스킬로도 대신 호출하지 않는다.
...
- `.env`에 `APP_IDENTIFIER_DEBUG`/`TEAM_ID`/`ASC_KEY_ID`/`ASC_ISSUER_ID`/`ASC_KEY_PATH`
...
- 위 점검을 통과했으면, **무엇을 하는지 정확히 요약해서 보여주고 승인을 받는다**:
  - `debug_beta` lane, scheme `WSS-iOS-DEBUG`, app identifier(`.env`의 `APP_IDENTIFIER_DEBUG` 값).
  - "아카이브 후 TestFlight 내부 테스터용으로 업로드됨(`distribute_external: false`)"임을 명시.
  - ✅ 2026-08-29 이 스킬로 실제 성공한 이력이 있다 — App Store Connect 앱 레코드 자체는 이미
    존재하므로 그 이유로 실패할 가능성은 낮다.
- 승인 없이 3단계로 넘어가지 않는다.

### 3. 실행 (승인 후)
- repo 루트에서: `bundle install`(Gemfile.lock 없을 때만) → `bundle exec fastlane debug_beta`.
...
- **실패**: fastlane이 뱉은 에러를 그대로 보여준다(추측으로 "아마 이래서 그럴 거예요"라고 뭉개지 않는다).
  ...
  - 프로비저닝 프로파일에 서명 문제 → `sync_dev_certificates`가 아니라 `match(type: "appstore")`를
    ...
    의심할 것.
  - App Store Connect 업로드 검증(`altool`) 실패(아이콘·버전 정보·기기군 등) → 한 번 실측으로 걷어낸
    적이 있다(`Projects/App/CLAUDE.md`의 "앱 아이콘·버전 정보는 시뮬레이터 빌드만으론 검증되지
    않는다" 항목 참고) — 같은 종류의 새 요구사항이면 그 문서 패턴을 따라 진단할 것.

## 원칙
- **Debug 스킴 전용** — Release는 `archive-release`. App Store 제출(`release` lane)은 어느 쪽도 대신
  호출하지 않는다.
...
- **실패를 뭉개지 않는다** — fastlane 에러를 그대로 보여주고, 원인이 코드 문제인지 계정/콘솔 설정
  문제인지부터 구분해서 짚는다(둘 다 실측 사례가 있다 — `Projects/App/CLAUDE.md`/`fastlane/Fastfile`
  주석 참고).
