...
- **근거는 항상 양쪽 경로**(V1 `repo 내부 경로:줄` · V2 `경로:줄`). 절대경로 금지. V1 경로는 `WSSiOS/Source/…` 형태(내부 경로라 이식성 있음).
  - ⚠️ **V2 `경로:줄` 인용은 코드가 바뀌면 조용히 밀린다** — V1은 참조 커밋에 고정되지만 V2는 살아 있는 코드라, 주석 두 줄만 더해도 그 아래 인용이 전부 어긋난다(#222에서 `DefaultFeedRepository:186→188` 6곳 실측). 줄 번호 옆에 **심볼명을 병기**해(`DefaultFeedRepository.swift:188 sortCriteria`) 번호가 틀려도 찾을 수 있게 하고, 인용된 V2 파일을 건드린 PR은 그 인용을 재검증한다.
