...
- 현재까지 `Core 3개`, `Domain 12개`, `Data 12개` 모듈에 더해 `UI 2개`·`Feature 9개`를 분리했고, Swift Testing과 `/domain-test` 기반 검증 흐름을 갖췄습니다.
...
`Feature`를 화면 단위(`NovelReviewFeature` → `FeedFeature` → `NovelDetailFeature` → `SearchFeature` → `KeywordFeature` → `LibraryFeature` → `HomeFeature`)로 점진 연결하고 있으며, 남은 화면과 `App` 조립(탭바·화면 전환)을 이어갈 계획입니다.
...
- `Feature`는 화면 단위로 점진 연결 중(`NovelReviewFeature`·`FeedFeature`·`NovelDetailFeature`·`SearchFeature`·`KeywordFeature`·`LibraryFeature`·`HomeFeature`)이며, 남은 화면과 `App` 조립을 단계적으로 이어갈 계획입니다.
...
├── HomeFeature           # 홈 탭 화면 (오늘의 발견·추천글·선호 장르 작품)
