...
`Feature`를 화면 단위(`NovelReviewFeature` → `FeedFeature` → `NovelDetailFeature`)로 점진 연결하고 있으며, 나머지 화면과 `App` 조립을 이어갈 계획입니다.
...
- `Feature`는 화면 단위로 점진 연결 중(`NovelReviewFeature`·`FeedFeature`·`NovelDetailFeature`)이며, 나머지 화면과 `App` 조립을 단계적으로 이어갈 계획입니다.
