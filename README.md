...
- 현재까지 `Core 3개`, `Domain 14개`, `Data 14개` 모듈에 더해 `UI 2개`·`Feature 13개`를 분리했고, Swift Testing과 `/domain-test` 기반 검증 흐름을 갖췄습니다.
...
`Feature`를 화면 단위(`NovelReviewFeature` → `FeedFeature` → `NovelDetailFeature` → `SearchFeature` → `KeywordFeature` → `LibraryFeature` → `HomeFeature` → `NotificationFeature`)로 점진 연결하고 있으며, 남은 화면과 `App` 조립(탭바·화면 전환)을 이어갈 계획입니다.
...
├── NotificationFeature   # 알림 목록·상세 화면
