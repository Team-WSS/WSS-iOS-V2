...
- **`QueryItemConvertible`은 빈 배열을 쿼리에서 통째로 생략한다**(#256 — 예전엔 `key=` 빈 값으로 나가 서버가 `[""]` 필터로 오해할 여지). "서버는 미전송과 동일 취급" 전제는 **실측으로 확인됨**(dev 서버 2026-09-09, `/novels/filtered`): 무필터를 생략/빈 값 두 방식으로 보내 `resultCount` 95,598·목록 완전 동일, 장르 하나 걸어도 38,944·목록 동일. 빈 값 직렬화로 되돌리지 말 것.
