...
- **에러 변환의 본진**: `NetworkingError.toRepositoryError()` — 401→`authenticationRequired`, 403→`forbidden`, 404→`notFound`, 5xx→`serverUnavailable`, decoding→`invalidData`. ⚠️ **`.unknown(URLError)`은 갈린다**(#244): `URLError.notConnectedToInternet`/`.networkConnectionLost`(진짜 오프라인)만 `networkUnavailable`, 그 밖의 통신 실패(타임아웃·DNS·취소 등)는 `unknown`(재시도 가능한 일반 오류)이다 — 화면이 "인터넷 연결 확인" vs "일시적 오류" 문구를 정확히 가르기 위함(전엔 통신 실패를 전부 `networkUnavailable`로 뭉갰다). `URLError` 검사는 도메인 무관한 Foundation 심볼이라 Core가 아니라 이 변환 지점 몫. (전 Data 모듈이 이걸 씀)
...
- `StorageKey` 추가 시 타입(`V`)을 정확히 — `UserDefaultsStorage`는 `as? V` 캐스팅이라 타입 불일치는 조용히 nil.
  - **사용자 개인 값 키를 새로 추가하면 `AuthData`의 `clearUserScopedCache`(세션 경계 정리 목록 — 로그아웃·탈퇴·로그인, #236)에도 반영할 것** — 컴파일러가 못 잡아주는 수동 동기화다(→ `AuthData/CLAUDE.md` 주의사항).
  - ⚠️ **`isRegistered`(#257) 같은 키는 "값의 의미"가 키가 아니라 읽는 쪽에 있다** — 이 키는 세 모듈이 나눠 만지고(AuthData 로그인=서버 isRegister 저장 / ProfileData 등록=true / SplashData 부트스트랩=읽기), **키가 없으면 "가입 완료"로 간주**하는 하위호환은 SplashData의 `?? true`가 소유한다. 즉 `StorageKey.swift`만 봐선 "키 없음=완료"·"세 모듈 분담"이 안 보이니, 이 키의 기본값 의미를 바꾸거나 소비처를 추가할 땐 `SplashDomain`/`AuthData`/`ProfileData` 주의사항을 먼저 볼 것.
...
- **`AppStorage.remove`(#236)는 protocol extension 기본 구현(`set(key, nil)`)이 있다** — 기존 conformer(테스트 목)를 안 깨뜨리기 위한 것일 뿐, 실제 저장소 구현은 `UserDefaultsStorage`처럼 명시적 `removeObject`로 오버라이드하는 게 정답. dict 기반 목이 `set(nil)`을 "빈 값 저장"으로 구현했다면 remove가 조용히 어긋나니, 삭제를 검증하는 테스트는 목의 remove 동작부터 확인할 것.
