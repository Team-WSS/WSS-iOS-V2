...
- ⚠️ **내 피드 `sortCriteria`는 `option.sortType.rawValue`를 그대로 싣는데, 이게 소문자 `recent`/`old`다**(`BaseDomain.SortType`이 `case recent`/`old`에 명시 rawValue가 없어서). **V1은 같은 엔드포인트에 대문자 `RECENT`/`OLD`를 보냈고**(`SortType.queryText`), **타 모듈은 서버 토큰을 `.rawValue.uppercased()`로 맞춘다**(`NovelMapper`의 읽기상태, `CollectionDetailQuery`의 정렬). 내 피드만 대문자화가 빠져 있어, 서버가 대소문자 민감이면 정렬이 조용히 서버 기본값으로 떨어진다(C2 2026-08-28 발견 — 서버 확인 대기, `docs/TODO.md` §9). 소소피드 옵션(`feedsOption`)은 `SosoFeedOption`이 `= "ALL"`/`"RECOMMENDED"`로 rawValue를 명시해 문제없다 — **명시 rawValue가 없는 열거형을 서버로 그대로 실을 때 이 함정을 의심할 것.**
