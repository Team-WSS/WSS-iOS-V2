...
- **`fetchNovelFeeds`의 `size: Int?`는 nil→`pageSize`(20) 폴백만 하고 상한 클램프는 일부러 하지 않는다**(#236) — 요청 크기 규칙은 Domain 정책 소유(호출자가 자른 값을 넘기는 계약)라 여기서 한 번 더 자르면 상한이 두 곳으로 갈라진다. (그 정책 `NovelFeedPageSizePolicy`는 소비자였던 작품 상세 전체 재조회와 함께 #256에서 삭제됐고, 현재 모든 호출자가 nil만 넘긴다 — 배관을 되살릴 때도 클램프는 Domain 쪽에 둘 것.)
...
- `UserFeedResponse.likerUsers`는 `FeedMapper`가 전혀 읽지 않고 실제 서버 응답과도 안 맞아 `getMyFeeds`/`getUserFeeds`의 JSON 디코딩이 실패하는 원인이었다 → 제거함. `UserFeedListResponse.feedsCount`(페이지네이션 전 전체 개수)는 실제 응답 필드가 맞다 — **#222부터 `FeedMapper.userFeeds`가 이 값을 `Paginated.totalCount`(BaseDomain, optional·기본 nil)에 실어** 내 피드 "n개의 기록" 표시가 로드된 배열 길이 대신 서버 전체 개수를 쓴다(V1 parity). **Response DTO는 형제 DTO를 베껴 짐작하지 말고, 실제 응답 바디로 검증할 것.**
