...
- **`fetchNovelFeeds`의 `size: Int?`는 nil→`pageSize`(20) 폴백만 하고 상한 클램프는 일부러 하지 않는다**(#236) — 현재 모든 호출자가 `nil`만 넘긴다(가변 size를 쓰던 정책 `NovelFeedPageSizePolicy`는 소비자였던 작품 상세 전체 재조회와 함께 #256에서 삭제됨). 향후 가변 size를 되살린다면 요청 크기 규칙(상한 클램프 포함)은 그때도 Domain 정책에 둘 것 — 여기서 한 번 더 자르면 상한이 두 곳으로 갈라진다.
