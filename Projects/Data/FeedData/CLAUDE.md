...
- **`fetchNovelFeeds`의 `size: Int?`는 nil→`pageSize`(20) 폴백만 하고 상한 클램프는 일부러 하지 않는다**(#236) — 서버 상한(100) 규칙은 `FeedDomain.NovelFeedPageSizePolicy` 소유라, 호출자(작품 상세 재진입 갱신)가 정책으로 자른 값을 넘기는 계약이다. 여기서 한 번 더 자르면 상한이 두 곳으로 갈라진다.
