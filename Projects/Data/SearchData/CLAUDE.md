...
- **상세탐색 필터(#185) 쿼리 파라미터**: `platformNames: [String]`(한글 표기, UI 라벨 "리디"↔서버값 "리디북스" 불일치 의도됨, `SearchMapper.mapNovelPlatformString` — 실서버 실측 2026-08-28: `platformNames=리디북스` 41,203건 / `리디` 0건 / 무필터 95,086건로 확인. ⚠️ **서버는 모르는 플랫폼 값에 에러 없이 0건을 준다** — 값 오타가 조용히 빈 결과로 새니 플랫폼 문자열을 바꿀 땐 반드시 실서버 결과 수로 검증)과 별점 범위 `novelRatingStart`/`novelRatingEnd: Float`(사용자 확정 필드명). 필터 없음(`ratingRange == nil`)이면 전체 범위(0.0~5.0)를 그대로 보낸다. (초기엔 단일값 `novelRating`도 함께 있었으나 #185 후반에 `SearchFilter.ratingThreshold` 자체가 제거되며 같이 빠졌다.)
- **`DetailSearchQuery.isCompleted`는 `Bool?`(optional)다 — 연재상태 미선택이면 nil로 둬 파라미터를 생략한다.** non-optional `Bool`이던 시절엔 미선택도 `false`로 나가 서버가 "연재중만"으로 해석해 **완결작 90%가 누락되는 회귀**가 있었다(실서버 검증 2026-08-28: `/novels/filtered` `false`=9,318·`true`=85,720·생략=95,038). `SearchMapper`가 `filter.publicationStatus.map { $0 == .completed }`로 만들어(`== .completed`가 nil을 false로 접던 걸 대체) 미선택이면 nil→`QueryItemConvertible`이 NSNull 제외로 자동 생략 → 전체 반환. **`== .completed`(nil을 false로 접음) 패턴으로 되돌리지 말 것** — 서재 `NovelMapper.myLibraryV2Query`와 동일 모델. 이 규칙은 `SearchMapperTests`(미선택→nil/완결→true/연재중→false)가 고정한다 — SearchData `Project.swift`에 `.tests` 타깃을 배선(#221)해 develop PR마다 CI가 자동으로 돌린다(Mock 불필요한 순수 매퍼 테스트라 `.testing` 없이 `.tests`만 둠).
