...
- **상세탐색 필터(#185) 쿼리 파라미터**: `platformNames: [String]`(한글 표기, UI 라벨 "리디"↔서버값 "리디북스" 불일치 의도됨, `SearchMapper.mapNovelPlatformString`)과 별점 범위 `novelRatingStart`/`novelRatingEnd: Float`(사용자 확정 필드명) — 기존 단일값 `novelRating`(`SearchFilter.ratingThreshold`용)과 공존한다. 필터 없음(`ratingRange == nil`)이면 전체 범위(0.0~5.0)를 그대로 보낸다.
