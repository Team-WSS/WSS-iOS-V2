...
- **`DetailSearchQuery.isCompleted`는 `Bool?`(optional)다 — 연재상태 미선택이면 nil로 둬 파라미터를 생략한다.** non-optional `Bool`이던 시절엔 미선택도 `false`로 나가 서버가 "연재중만"으로 해석해 **완결작 90%가 누락되는 회귀**가 있었다(실서버 검증 2026-08-28: `/novels/filtered` `false`=9,318·`true`=85,720·생략=95,038). `SearchMapper`가 `filter.publicationStatus.map { $0 == .completed }`로 만들어(`== .completed`가 nil을 false로 접던 걸 대체) 미선택이면 nil→`QueryItemConvertible`이 NSNull 제외로 자동 생략 → 전체 반환. **`== .completed`(nil을 false로 접음) 패턴으로 되돌리지 말 것** — 서재 `NovelMapper.myLibraryV2Query`와 동일 모델. ⚠️ **매퍼 회귀 테스트가 아직 없다**(SearchData에 `.tests` 타깃 미배선) — 타깃 배선 시 미선택→nil/완결→true/연재중→false로 고정할 것.
