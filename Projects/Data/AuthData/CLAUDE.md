...
- ⚠️ **사용자 스코프 캐시 정리(`clearUserScopedCache`)는 세 성공 경로에서 부르고, 키 목록의 완전성은 컴파일러가 못 지켜준다**(#236) — 로그아웃·탈퇴(세션 종료) + **로그인(새 세션 시작 직전)**. 로그인 쪽이 필요한 이유: 401 만료·부트스트랩 `.intro` 낙착처럼 **서버가 세션을 끊은 경로는 종료 시점 정리를 지나지 않아**, 다른 계정으로 로그인한 뒤 프로필 동기화가 실패하면 이전 사용자의 값이 비친다(#236 리뷰에서 발견 — 로그인 선제 삭제가 모든 경로의 수렴점). `StorageKey`에 **사용자 개인 값** 키(userID·nickname·characterID·gender·birthYear·myLibraryFilter 류)를 새로 추가하면 이 목록에도 같이 넣어야 한다 — 빠뜨리면 빌드는 초록인데 **계정을 갈아탄 다음 사용자 화면에 이전 사용자의 값이 비친다**(홈 "{닉네임}님을 위한 추천글"이 첫 노출 지점). 서버 호출이 실패하면 세션 경계를 안 넘은 것이라 토큰과 마찬가지로 캐시도 보존.
...
- ⚠️ **`.isRegistered`(온보딩 완료 여부, #257)는 `clearUserScopedCache` 목록에 있으면서도 `login`에서 clear 직후 서버 `isRegister` 값으로 다시 세팅한다** — 다른 사용자 스코프 키(닉네임 등)는 지우기만 하고 이후 async 동기화가 채우지만, 이 값은 로그인 응답에만 있어 **동기적으로** 되채운다. 그래서 `login`의 `clearUserScopedCache() → appStorage.set(.isRegistered, ...)` 순서를 뒤집으면 새 값이 지워진다. 온보딩 완료 시 true로 올리는 건 ProfileData(`registerProfile`), 읽는 건 SplashData 부트스트랩 게이트다(세 모듈이 이 한 키를 나눠 만진다).
