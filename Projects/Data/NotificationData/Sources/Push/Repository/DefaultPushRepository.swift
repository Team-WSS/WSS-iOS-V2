...
public struct DefaultPushSettingRepository: PushSettingRepository {
    ...
    public func loadPushPreference() async throws(RepositoryError) -> PushPreference {
        do {
            ...
            logger?.logSuccess(action: "loadPreference")
            return result
        } catch let error as NetworkingError {
            logger?.logNetworkError(action: "loadPreference", error: error)
            throw error.toRepositoryError()
        } catch {
            logger?.logUnknownError(action: "loadPreference", error: error)
            throw .unknown
        }
    }
    ...
    public func updatePushPreference(_ preference: PushPreference) async throws(RepositoryError) {
        do {
            ...
            logger?.logSuccess(action: "updatePreference")
        } catch let error as NetworkingError {
            logger?.logNetworkError(action: "updatePreference", error: error)
            throw error.toRepositoryError()
        } catch {
            logger?.logUnknownError(action: "updatePreference", error: error)
            throw .unknown
        }
    }
    ...
    public func registerDeviceToken(_ token: DevicePushToken) async throws(RepositoryError) {
        do {
            ...
            logger?.logSuccess(action: "registerToken")
        } catch let error as NetworkingError {
            logger?.logNetworkError(action: "registerToken", error: error)
            throw error.toRepositoryError()
        } catch {
            logger?.logUnknownError(action: "registerToken", error: error)
            throw .unknown
        }
    }
}
