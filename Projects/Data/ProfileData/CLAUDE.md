...
- **존재하지 않는/탈퇴한 유저(`USER-018`) 감지도 같은 방식** — `fetchUserProfile`의 `catch let error as NetworkingError`에서 `body?.code == "USER-018"` → `RepositoryError.notFound`을 throw한다(#222 V1 parity). UserPageFeature가 이 `.notFound`를 "존재하지 않는 유저예요" 전용 화면으로 분기해(일반 로드 실패=`NetworkErrorView` 재시도와 구분) 재시도 무한반복 회귀를 막는다. USER-015와 달리 프로필 조회 메서드에만 붙인 이유: 유저 페이지 로드가 프로필을 먼저 `await`하므로 그 지점에서 잡으면 충분(장르·작품취향까지 붙일 필요 없음).
- **`registerProfile` 성공 시 `localStorage.set(.isRegistered, true)`로 온보딩 완료를 기록한다(#257).** 이 서버 호출이 서버 측 `isRegister`를 true로 바꾸는 시점이라 여기가 정확한 로컬 반영 지점이다 — 로그인 때 AuthData가 false로 심고, 여기서 true로 올리며, 다음 콜드 스타트에 SplashData 부트스트랩 게이트가 이 값을 읽어 홈/인트로를 가른다(한 키를 세 모듈이 나눠 만지는 크로스 모듈 결합 — `registerProfile`만 보면 이 auth/온보딩 플래그를 건드리는 게 안 보인다). 다른 로컬 캐시와 동일하게 `service.postRegisterProfile` **성공 뒤에만** 쓴다.
- **`ProfileMapper.profile(from:)`(`fetchUserProfile`이 씀)의 `genrePreferences` 매핑은 `try?`(`compactMap`)로
  관대하게 처리한다**(#255 QA) — 이 필드는 `UserPageFeature`가 실제로 안 쓰는 죽은 값이다(장르 뱃지는
  `fetchGenrePreferences`라는 **다른** 엔드포인트로 채움). `try`로 엄격 매핑하면 서버가 이 화면이 모르는
  장르 문자열 하나만 보내도 `MappingError`로 **전체 프로필 매핑이 실패**해, 정작 필요한 닉네임/소개/이미지까지
  못 쓰게 된다(실측으로 재현되진 않았으나 비공개 유저 상단 정보 노출 버그를 고치며 방어적으로 함께 고침
  — `UserPageFeature/CLAUDE.md`의 "비공개 프로필" 항목 참고). 새로 이 응답에 필드를 추가로 매핑할 때
  **실제로 화면이 쓰는 값만 엄격(`try`)하게, 안 쓰는 값은 관대(`try?`)하게** 매핑할 것.
