...
- **피드 작성 완료 → 피드 탭 목록 재로드는 `FeedRootView` 로컬 `@State`(`needsFeedListReloadForCreatedFeed`)
  + `makeSosoFeedView(needsReloadForCreatedFeed:)` Binding 채널이다**(#256 — 예전 앱 전역 `@Observable` 카운터
  `FeedListInvalidation`은 제거됐다). **연필 아이콘(`.createFeed`) 작성 성공 복귀에만 켜진다**: `FeedRootView`의
  `createFeedView(connectedNovel:reloadsFeedListOnSubmit:)` 헬퍼가 연필 경로만 true를 받아 `onSubmitted`에서
  플래그를 켜고, 피드 목록의 복귀 `onAppear`가 소비해(true→false) 두 목록을 초기 로드처럼 다시 받는다.
  ⚠️ **작품 상세 경유 작성(`.createFeedFromNovel`, 4탭 전부)은 피드 탭에 신호를 보내지 않는다**(사용자 확정 —
  그 작품 상세가 자기 피드 섹션을 리셋하고, 피드 탭 반영은 당겨서 새로고침에 맡긴다). ⚠️ **수정
  (`makeEditFeedView`)의 `onSubmitted`에도 붙이지 말 것** — 수정은 목록이 다녀온 셀을 상세로 동기화하고,
  재로드하면 수정 후 복귀마다 스크롤이 튄다. `CrossScreenFeedback`(그 탭에서 한 번 보여주는 토스트)과 역할이
  다르니 합치지 말 것.
