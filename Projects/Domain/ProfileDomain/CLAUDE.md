...
- `fetchNovelPreferences`는 NovelDomain과 동일하게 **캐시 키워드 주입** 필요 — `LoadNovelPreferencesUseCase`가
  `KeywordRepository`를 직접 잡지 않고 `BaseDomain.LoadTotalKeywordsUseCase`를 통해 읽는다(2026-09-12,
  캐시 미스 시 재시도 정책은 그 UseCase 소유 — `BaseDomain/CLAUDE.md` 참고).
