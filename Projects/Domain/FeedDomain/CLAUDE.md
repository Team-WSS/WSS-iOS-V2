...
- **`TotalFeed.updated(from: FeedDetail)`**(2026-09-03) — 피드 탭 목록이 재진입에 목록 대신 **다녀온 셀만** 상세로 갱신할 때 쓴다(`FeedFeature/CLAUDE.md` 화면 동작 계약). 본문·좋아요·댓글수·스포일러/공개/수정 여부·연결 작품·이미지(썸네일=첫 장, 개수)는 상세 값을 따르고 **`feedId`·`createdDate`·`author`·`isMyFeed`는 로컬을 유지**한다 — `isMyFeed`는 상세 응답에 없고, `author`는 내 피드 목록이 프로필 조회로 덧씌운 값이라 상세의 author로 되돌리면 그 조립이 풀린다. 상세의 `feedImageURLs`가 `[URL?]`라 썸네일은 `first ?? nil`(이중 옵셔널 평탄화).
  - ⚠️ **연결 작품 별점(`ConnectedNovel.rating`)은 "작품 전체 평점"이 아니라 "글쓴이 별점"이다** — 그래서 이 갱신은 `detail.connectedNovel?.basicInfo`를 통째로 넣지 않고, id·제목·장르만 `basicInfo`에서 가져오되 **`rating`은 `feedWriterRating`으로** 채운 `ConnectedNovel`을 새로 만든다(2026-09-11 버그 수정 — 예전엔 `basicInfo`를 통째로 넣어 상세 복귀 순간 별점이 전체 평점으로 바뀌었다). **필드명 불일치 주의**: 같은 "글쓴이 별점"을 목록 API는 `novelRating`으로, 상세 API는 `feedWriterNovelRating`으로 내려준다(상세의 `novelRating`은 전체 평점 — `FeedMapper.feedDetail`이 `basicInfo.rating`=전체, `feedWriterRating`=글쓴이로 가른다). 상세 화면(`FeedDetailLinkNovelBlock`)은 두 값을 나란히 보여줘 `basicInfo.rating`이 계속 필요하므로 **매퍼를 고치지 말고 이 갱신 지점에서만** 별점을 글쓴이 값으로 집어온다. `rating`이 `nil`/0이면(글쓴이 미평가) 목록 셀(`WSSLinkNovelView`)이 별점 표기를 생략한다.
- **목록 4종 중 `fetchNovelFeeds`만 `size: Int?`를 받는다**(#236) — 작품 상세의 재진입 조용한 재조회("보던 개수만큼 통째 교체")를 위해 뚫었던 배관인데, **그 재조회가 #256에서 셀 동기화로 교체되며 요청 크기 정책(`NovelFeedPageSizePolicy`)은 테스트와 함께 삭제됐고 현재 모든 호출자가 nil(기본 페이지 크기)만 넘긴다.** 파라미터 자체는 Repository 계약이라 남겨뒀다 — 가변 size가 다시 필요해지면 이 배관을 재사용하되, "보던 개수 window 교체"는 새 글 유입 시 기존 글이 창 밖으로 밀리는 결함이 판정된 방식이니 되살리지 말 것(`NovelDetailFeature/CLAUDE.md` 2026-09-08 재판정).
