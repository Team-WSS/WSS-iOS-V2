...
public final class DefaultBootstrapAppUseCase: BootstrapAppUseCase {
    ...
    public func execute() async -> BootstrapOutcome {
        ...
        // 2-1. 온보딩(가입) 게이트 — 세션은 있으나 온보딩을 끝내지 않은 유저(로그인 직후 앱 종료 등)는
        //      인트로로 되돌린다(#257). 로그인만 하고 나면 토큰은 저장돼 있어 세션 게이트는 통과하는데,
        //      isRegister=false(로컬 캐시)라 홈으로 보내면 프로필 없는 반쪽 상태로 진입한다. 여기서 끊어
        //      재로그인→온보딩으로 유도한다. 값이 없으면(기존 유저) 완료로 간주해 통과(구현이 `?? true`).
