...
- `.intro`로 낙착해도 **이미 던진 부수 태스크는 되돌리지 않는다**. 4종 중 3종은 죽은 세션에서 실패하고 끝나
  무해하지만, **홈 프리페치만 다르다** — `/novels/popular`·`/feeds/popular`가 `.usesTokenIfAvailable`이라
  익명으로도 200을 받아 store를 채운다. 세션 소실 → 인트로 → 재로그인 뒤 첫 홈 로드가 **런치 시점 데이터를
  소비**할 수 있다(표시물이 인기작·뜨는 글이라 개인화 유출은 아니고, 실해는 "몇 분 묵은 데이터" 수준).
  프리페치 3종 중 **taste(`/novels/taste`)만은 `requireToken`이라 이 경로에서 실패해 슬롯이 비어 있다**
  (fail-closed) — 개인화 슬롯이 세션 경계를 넘는 일은 없다. 이 성질이 taste를 프리페치에 넣을 수 있었던
  전제이니(2026-08-31), taste를 `usesTokenIfAvailable`로 되돌리지 말 것.
