...
- 게이트 순서·실패 정책은 의도된 결정이다(버그 아님):
  ...
  - **게이트 조회 실패 = 통과** — 서버 장애가 앱을 잠그면 안 된다(사용자 확정, #225). 약관 조회 실패도 동의로 간주.
  ...
  - **온보딩 게이트(#257)는 세션 게이트 바로 뒤** — 세션(토큰)은 있으나 온보딩(가입) 미완료면 `.intro`로 보낸다.
    "세션이 있는데 인트로"라 언뜻 모순 같지만, **로그인만 하고 온보딩 도중 앱을 종료한 유저**를 위한 것이다(토큰은 저장돼
...
- `.intro`로 낙착해도 **이미 던진 부수 태스크는 되돌리지 않는다**. 죽은 세션에선 4종 전부 실패하고 끝나
  ...
  부활한다**(개인화인 taste는 유출 성격까지 생김). 되돌리지 말 것. 세션 전환 시 store 교체는
  "유효 토큰으로 채워진 뒤 소비 전에 세션이 바뀌는" 좁은 레이스 대비로 **#236에서 구현됐다** —
  `ContentView.resetToOnboarding`이 세션이 끝나는 모든 경로에서 `AppDependencies`를 통째로 재조립해
  store가 인스턴스째 교체된다(`App/CLAUDE.md`).
