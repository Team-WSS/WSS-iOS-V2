...
public enum DeepLink: Hashable, Sendable {
    ...
    /// 알림 상세(공지 등) — 푸시 알림(`view=notificationDetail`, `notificationId`로 도착, #243 실측 확정) /
    /// 테스트용 `websoso://notifications/{notificationId}`. 앱 내 알림 목록 셀 탭도 같은 화면으로 가지만
    /// (`NotificationDeeplink.notificationDetail`) 그건 API 응답을 푸는 경로라 이 타입과 무관하다.
    ...
    /// 푸시 payload의 `view` 값 → 화면 종류. 서버가 `view`에 맞는 id만 채워 보낸다(나머진 빈 문자열/생략 —
    /// 예: 공지는 `notificationId`만 채우고 `novelId` 키 자체가 없다, #243 실측). 그래서 판별자는 `view`다.
    ...
    public var url: URL? {
        ...
        case .novelDetail, .feedDetail, .notificationDetail:
    ...
    public var kakaoExecutionParameters: [String: String] {
        ...
        case .novelDetail, .feedDetail, .notificationDetail:
            // 카카오 공유 카드는 컬렉션 전용 — 작품/피드/알림 상세 딥링크는 푸시로만 온다.
            return [:]
        }
    }
    ...
    public init?(url: URL) {
        ...
        case Host.notifications:
            guard let id = Self.singlePathID(from: url) else { return nil }
            self = .notificationDetail(NotificationID(id))
        case Host.kakaoLink:
    ...
    public static func fromNotificationPayload(_ payload: [String: String]) -> DeepLink? {
        ...
        case NotificationView.notificationDetail:
            guard let id = parseID(payload[NotificationKey.notificationID] ?? "") else { return nil }
