...
- **`DeepLink`**(#228) — 딥링크의 **생성(`url`·`kakaoExecutionParameters`)과 파싱(`init?(url:)`)을 한 타입에** 둔다. 만드는 쪽(Feature의 공유 버튼)과 받는 쪽(App `onOpenURL`)이 같은 규칙을 봐야 어긋나지 않아서다 — 새 딥링크 화면을 추가할 땐 case를 늘리고 **양쪽을 같은 커밋에서** 고칠 것(App의 4탭 Root `switch`가 exhaustive라 컴파일러가 잡아준다). `AppURL`과 같은 이유로 여기 있다(Feature가 참조 가능한 유일한 공통 자리). 받는 형식은 둘: `websoso://<host>/<정수 id>`(커스텀 스킴, App `Info.plist` 등록)와 `kakao{APP_KEY}://kakaolink?collectionId=<id>`(카카오톡 공유 카드를 탭하면 카카오톡이 만드는 URL). **세 번째 수신 경로는 FCM 푸시 payload**(`fromNotificationPayload`, #243) — URL이 아니라 `[String:String]`이고 판별자는 **`view` 문자열**(`novelDetail`→작품 상세 / `feedDetail`→피드 상세 / `notificationDetail`→알림 상세, 공지 등), 서버가 `view`에 맞는 id(`novelId`/`feedId`/`notificationId`)만 채운다(실측: 공지 push는 `notificationId`만 채우고 `novelId` 키가 아예 없다 — 그래서 id 부재가 아니라 `view`가 판별자다). 이 세 case는 App만 푸시로 받으니 outbound(`url`/`kakaoExecutionParameters`) 대상이 아니다(전부 nil/[:], 테스트용 `websoso://novels|feeds|notifications/<id>` 파싱만 지원). ⚠️ **`kakaolink` host는 스킴을 검사하지 않는다** — 앱 키는 App 설정에만 있어 이 모듈이 모르고, 어차피 등록된 스킴만 앱에 도착한다. 스킴·host는 대소문자 무시. **ID는 양의 정수만 인정**(`0`·음수는 nil, 리뷰 반영) — 통과시키면 존재할 수 없는 ID로 상세를 열어 404 실패 화면부터 보게 된다. **`websoso://` 생성(`url`)·`kakaoExecutionParameters` 둘 다 지금 프로덕션 호출자가 없다**(#241 — 컬렉션 공유 카드가 Kakao 기본 템플릿(`Link`+실행 파라미터)에서 콘솔 커스텀 템플릿(`templateArgs`의 `${collectionId}` 변수 치환)으로 전환되면서 `kakaoExecutionParameters` 호출부가 사라졌다. `websoso://`는 그 전인 2026-08-29부터 이미 미사용). 테스트(`DeepLinkTests`)만 남아있고, 위 kakaolink URL 자체(파싱 쪽 `init?(url:)`)는 여전히 프로덕션이 받는 형식이라 지우지 말 것 — 시뮬레이터 실측(`simctl openurl`)·Universal Link 전 단계용으로 스킴 등록과 함께 남겨둔 것이니 생성 쪽(`url`/`kakaoExecutionParameters`) 미사용만으로 "죽은 코드"라 지우지 말 것.
- **온보딩 힌트 서브도메인** (`Onboarding/`, #221): `OnboardingHint`(1회성 힌트 종류 enum) + `OnboardingHintRepository` + `OnboardingHintUseCase`. 앱 전역에서 "이 힌트를 봤는가"를 로컬에 저장/조회하는 **범용** 계약 — 화면이 늘면 `OnboardingHint` 케이스만 더한다(힌트마다 저장소/UseCase를 새로 만들지 않는다). ⚠️ **`async throws`가 아니라 동기·비-throwing**이다(순수 로컬 플래그라, 화면 첫 프레임에 표시 여부를 동기로 정해야 깜빡임이 없다). ⚠️ **`OnboardingHint.rawValue`는 저장 키의 일부**라 한 번 정하면 바꾸지 않는다(바꾸면 이미 본 유저에게 다시 뜬다). 구현은 `BaseData`의 `DefaultOnboardingHintRepository`(UserDefaults). 첫 사용처는 작품 상세 첫 진입 평가 안내(`.novelDetailReview`, `NovelDetailFeature`).
- **앱 리뷰 요청 서브도메인** (`AppReview/`, #221): `AppReviewRequestRepository`(순수 저장소) + `AppReviewRequestUseCase`(정책). 앱스토어 평점 프롬프트를 **언제 띄울지** 게이팅하는 **범용** 계약(피드·감상평이 하나를 공유 → 앱 전역 기준). 온보딩처럼 **동기·비-throwing**(저장 성공 직후 동기 판정). 정책은 전부 `DefaultAppReviewRequestUseCase`가 소유 — `recordEngagement()`가 긍정 완료를 세고, `shouldRequestReview()`가 **이번 버전 누적 참여 ≥ 임계치(기본 3, init 파라미터) AND 이번 앱 버전 미요청**일 때만 true, 호출 후 `markReviewRequested()`로 현재 버전 각인. ⚠️ **참여 카운트는 앱 버전(`CFBundleShortVersionString`)이 바뀌면 0부터 다시 센다** — `engagementCountVersion`(카운트가 쌓인 버전)을 현재 버전과 비교해 다르면 0으로 취급(`currentVersionEngagementCount`). 누적이면 한 번 임계치를 넘긴 뒤 새 버전마다 첫 저장에 바로 떠 임계치가 죽으므로, "매 버전 임계치를 다시 채운 활발한 유저에게만 버전당 1회"로 만든 것. 버전 리셋 판정을 **저장소가 아니라 UseCase가** 하는 이유는 Domain에서 테스트 가능하게 하려는 것(저장소는 값만 보관). ⚠️ **enum 없이 2파일**(온보딩과 달리 힌트 종류가 없는 단일 개념)이고, **실제 StoreKit 호출은 여기 없다** — 프롬프트는 Feature의 View가 `@Environment(\.requestReview)`로 띄우고, 이 스택은 "지금 물어볼까?"만 판정한다. 구현은 `BaseData`의 `DefaultAppReviewRequestRepository`(UserDefaults + Bundle 버전).
