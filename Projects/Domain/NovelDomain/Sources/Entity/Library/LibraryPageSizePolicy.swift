...
public enum LibraryPageSizePolicy {
    ...
    ///
    /// 이 크기로 받으면 서버에 그만큼 있는 한 **목록 개수가 유지된다**(짧아지지 않으므로 스크롤도 안 튄다).
    ...
    /// 없어도 스크롤 한 번이면 복구되는 수준이라 **매끄러움을 위한 보정**이지 정확성 문제는 아니다.
    ///
    ...
    ///
    /// ⚠️ **개수 차이로만 판단하므로 "앞 추가 + 뒤 삭제"가 정확히 상쇄되면 보정이 생략된다**(delta 0).
    /// 그래도 **해롭지 않다** — 1차가 보던 개수만큼 채워 오므로 목록 길이·스크롤 위치는 그대로고,
    /// 앞에 끼어든 만큼 window 끝에서 밀려난 항목은 커서가 정확해 **한 번 더 스크롤하면 이어 붙는다**.
    /// 삭제된 항목이 사라지는 것도 맞는 동작이다. 정확히 잡으려면 개수가 아니라 "직전 마지막 항목이
    /// 1차 응답에 남아 있는지"를 봐야 해 정책이 아이템까지 알아야 하는데, 그만한 값어치가 없다.
    ///
