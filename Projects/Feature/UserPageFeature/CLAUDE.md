...
- **CSV 이벤트 `mypage_man`/`mypage_woman`/`mypage_age`("프로필 편집 > 성별/나이 선택")는 여기가 아니라
  ...
  ⚠️ **`mypage`/`other_mypage` 둘 다 `load()`의 `!hasLoadedContent`(최초 진입) 분기에서만 쏜다**(#250 리뷰로
  정정) — 이 화면들은 탭 콘텐츠·push 복귀 모두 재조회 자체엔 최초 1회 가드가 없지만(`Feature/CLAUDE.md`
  "탭 콘텐츠는 복귀마다 갱신" 규약), 트래킹은 그 재조회 가드와 별개로 `hasLoadedContent`/`hasLoaded`
  체크를 앞에 둬 재진입마다 중복 집계되지 않게 한다.
