...
- **Demo 실서버 모드는 키워드 캐시가 준비된 뒤에 화면을 세운다**(내 서재·타유저 서재 **둘 다**, `ProgressView`로 가드) — 원래는 캐시 미스 시 UseCase가 `try?` + `?? []`로 조용히 빈 값 폴백만 하던 시절, "키워드가 안 나온다"는 오진을 피하려는 방어였다. ⚠️ **2026-09-12부터 이 문제 자체가 `LoadMyLibraryUseCase`/`LoadUserLibraryUseCase`가 `BaseDomain.LoadTotalKeywordsUseCase`를 통해 캐시를 읽도록 바뀌며 완화됐다** — 캐시 미스면 그 UseCase가 자체적으로 서버 재동기화 후 재조회하므로, 재시도까지 실패하는(오프라인 등) 드문 경우가 아니면 더 이상 "에러 없이 키워드만 통째로 빈 채" 그려지지 않는다. Demo의 이 가드는 여전히 무해하고(첫 로드 지연을 매끄럽게 가림) 걷어낼 필요는 없지만, **더 이상 "안 하면 반드시 깨지는" 필수 사항은 아니다** — 이유를 착각해 다른 화면에 그대로 복제하지 말 것(`BaseDomain/CLAUDE.md`의 `LoadTotalKeywordsUseCase` 항목이 정본).
