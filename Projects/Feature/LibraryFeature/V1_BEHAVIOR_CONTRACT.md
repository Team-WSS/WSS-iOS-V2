...
6. 🔧/🗑 **Improve(6종)·Delete(최적화)** — 타유저 서재 정렬 **2종(최신/오래된) → 6종**으로 확장, 그리고 **1페이지 내 정렬 시 클라이언트 배열 뒤집기** 최적화 제거. → [3.2](#32-정렬)
7. 🗑 **Delete** — 타유저 서재 빈 화면의 **"작품 찾기" CTA 제거**. → [3.3](#33-로드페이지네이션빈-화면에러)
...
- 🔧 **Improve** — 정렬 **2종 → 6종**. V1 타유저 서재는 드롭다운으로 **최신순/오래된순 2종**만. V2는 내 서재와 같은 `LibrarySortSheet` 6종을 재사용.
...
- 🗑 **Delete** — **1페이지 내 정렬 최적화**: 총 개수가 페이지 크기 이하(`isNotLoadable`)면 정렬 변경 시 서버를 다시 부르지 않고 **로컬 배열을 뒤집기**만 한다.
...
- 🗑 **Delete** — 빈 화면의 **"작품 찾기" CTA**. V1 빈 화면엔 검색으로 가는 버튼이 있었다.
