...
- **다른 화면에 콘텐츠로 얹히기(#185)**: `KeywordFeatureFactory.makeSearchKeywordView`가 `initialSelectedKeywords`(진입 시 이미 선택된 키워드 시딩), `showsBottomActionBar`(기본 `true` — 독립 화면 자체 하단바; `false`면 감추고 호출부가 자기 CTA를 쓴다), `onSelectionChanged`(선택이 바뀔 때마다 실시간 호출, 확정 버튼 불필요), `onComplete`(하단 "N개 선택" 확정 시, `showsBottomActionBar: false`면 호출 안 됨)를 받는다. **이 화면은 스스로 dismiss하지 않는다** — 독립 진입 시 닫는 것도 호출부 책임(`LibraryFilterSheet`의 `onApply` 패턴과 동일). `SearchFeature`의 상세탐색 필터 "키워드" 탭이 `showsBottomActionBar: false` + `onSelectionChanged`로 이 화면의 **콘텐츠만**(카테고리 브라우징+검색+선택, 하단바 제외) 자신의 탭 안에 얹어 재사용한다 — Feature 간 직접 의존 없이(Sources는 KeywordFeature를 모름), App/Demo가 콘텐츠 빌더 클로저를 조립해 값으로 건네는 방식(`SearchFeature`의 `KeywordTabContentBuilder` 참고).
