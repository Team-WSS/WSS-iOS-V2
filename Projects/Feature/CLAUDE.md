...
- ⚠️ **다만 표현 수단은 화면 구조를 따른다.** 서재는 목록을 걷어내고 재시도 버튼이 달린 `NetworkErrorView`로 대체하지만, `NovelDetail` 피드 탭은 **재시도 버튼 없는 빈 상태 문구**뿐이라 목록이 있는 상태(더보기 실패)에서는 실패를 알릴 자리가 없다 → 그 한 경로만 토스트로 남아 있다. **규칙을 어긴 게 아니라 UI가 아직 없는 것**이니, 통일하려면 재시도 UI를 먼저 정해야 한다.
