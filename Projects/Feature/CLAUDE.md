...
- `ModuleType.feature` enum에 선언만 있고 `Project.swift`가 없는 case(`home`, `collection`, `setting` — 디렉토리는 있어도 빈 폴더)는 아직 미구현. 실제 모듈: `NovelReviewFeature`, `FeedFeature`, `NovelDetailFeature`, `MypageFeature`, `SearchFeature`. `SearchFeature`는 소소픽 섹션만 UseCase 연동, 나머지 섹션은 정적 UI인 중간 상태 — 자세한 내용은 `SearchFeature/CLAUDE.md` 참고.
