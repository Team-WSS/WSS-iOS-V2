...
- `ModuleType.feature` enum에 선언만 있고 `Project.swift`가 없는 case(`home`, `collection`, `setting` — 디렉토리는 있어도 빈 폴더)는 아직 미구현. 실제 모듈: `NovelReviewFeature`, `FeedFeature`, `NovelDetailFeature`, `MypageFeature`, `SearchFeature`. `SearchFeature`는 소소픽·최근 검색어·키워드 검색(인기 키워드) 섹션이 UseCase 연동됐고, 나머지(장르별 검색·검색 실행)는 정적 UI인 중간 상태 — 자세한 내용은 `SearchFeature/CLAUDE.md` 참고.
