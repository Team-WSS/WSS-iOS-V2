...
- **View→VM 입력은 오직 `viewModel.handle(.xxx)`** (생명주기도 액션: `onAppear → .load`). `state`는 `private(set)` → 직접 변경 ❌.
  - **예외: `viewModel.track(.xxx)`(애널리틱스, #249)** — `state`를 건드리지 않는 순수 pass-through라 `handle(_:)`을 거치지 않고 View가 직접 호출한다(정본: `Projects/Core/Analytics/CLAUDE.md`).
...
- **탭 콘텐츠는 탭 복귀마다 갱신한다** — 홈·마이페이지·서재 모두 밖에서 바뀐 값(추천·알림·프로필, 작품 상세에서 고친 별점·읽기상태)을 다시 비춰야 해서다. 구 WSSiOS도 `viewWillAppear`마다 다시 불렀다. **`.load`에 "최초 1회" 가드를 넣지 말 것**(서재가 `hasLoaded`로 그렇게 돼 있었으나 걷어냈다) — 중복 요청은 진행 중인 Task 가드가 막는다.
  ...
  - ⚠️ **예외: 피드 탭(`FeedFeature/SosoFeedView`)은 복귀마다 갱신하지 않는다**(2026-09-03 사용자 확정). 커서 목록이라 다시 받으면 20개로 줄어 깊이 스크롤한 위치가 튀고, "보던 개수만큼 재조회"도 상한 100에서 잘린다 — 그래서 **다녀온 셀만 상세 API로 동기화**하고 전체 최신화는 당겨서 새로고침에 맡긴다(V1도 진입 1회 로드). 새 글은 피드 탭 연필 아이콘 작성 성공 복귀 신호(App 탭 Root 로컬 Binding, #256)로만 들어온다. 화면 안 서브탭(내 피드↔소소피드) 전환도 같은 이유로 재조회하지 않는다 — 두 리스트를 상시 mount해 탭별 스크롤 깊이까지 보존한다(2026-09-09 확장). 이 예외를 다른 탭에 복제하려면 그 화면에 pull-to-refresh가 있는지부터 볼 것. → [FeedFeature](FeedFeature/CLAUDE.md) 화면 동작 계약.
...
- ⚠️ **자동 전환 캐러셀을 페이지형 `TabView`(`.tabViewStyle(.page)`)로 만들지 말 것** — UIKit 페이징 브리지가 두 겹으로 배신한다: ① `.ignoresSafeArea()` 조상 아래에선 두 번째 페이지부터 콘텐츠가 상태바 높이만큼 위로 솟구치고(시뮬레이터 실측), ② 레이아웃이 정상이어도 **실기기에선 `withAnimation` 프로그램적 `selection` 전환이 엉뚱한 이웃 페이지로 애니메이션돼 내부 페이지와 `selection`이 어긋난다**(인디케이터만 돌고 이미지는 안 넘어감 — 실기기 실측. 시뮬레이터와 증상이 달라 시뮬레이터 검증만으론 오진한다). 대신 iOS 17 순수 SwiftUI 페이저(`ScrollView(.horizontal)` + `scrollTargetLayout()` + `scrollTargetBehavior(.paging)` + `scrollPosition(id:)`)로 만든다 — 정본은 온보딩 인트로 배너(→ [OnboardingFeature](OnboardingFeature/CLAUDE.md)).
...
- ⚠️ **`.navigationTitle`은 WSS 커스텀 폰트/컬러를 못 먹인다**(UIKit 네비바 기본 타이틀 뷰라서) — 대부분의 화면이 그래서 `.navigationTitle` 대신 `toolbarContent`의 `ToolbarItem(placement: .principal)`에 `Text(title).applyWSSFont(...)`를 직접 넣는다(`SettingAccountInfoView`·`NotificationSettingView` 등 다수, `NovelReviewView`도 이걸로 교체). 새 화면에서 타이틀이 기본 시스템 폰트로 나오면 이 패턴으로 바꿀 것.
