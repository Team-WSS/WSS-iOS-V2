...
- 진입점 (#201부터 — **화면 간 이동은 전부 App이 조립한다.** 예외는 `makeWithdrawFlowView` 하나뿐,
  ...
  `SettingRoute`/`SettingAccountInfoRoute`/`NotificationSettingRoute`/`NovelNotificationListRoute`, 정본 `Sources/Navigation/SettingRoutes.swift`).
  "저장됨" 계열 토스트(성별/나이 변경,
...
- **완결 알림/휴재 복귀 알림 목록(`NovelNotificationListView`, #188)** — `NotificationSettingView`의 두 row에서 push. **화면 구조가 완전히 같아**(사용자 확정) 별도 View 두 개를 만들지 않고 `NovelNotificationType`(`.completion`/`.hiatusReturn`)만 다르게 주입해 같은 View/VM을 재사용한다 — `SettingFeatureFactory`에 `makeCompletionNotificationListView`/`makeHiatusReturnNotificationListView` 두 진입점은 있지만 내부에서 같은 `NovelNotificationListView`(제목만 다름)를 만든다.
  ...
  - 삭제는 `WSSAlertType.deleteNovelNotificationSubscriptions(summary:)`(신규, `WSSComponent`) 확인 알럿을 거친다. `summary`는 **목록에 보이는 순서 기준 첫 선택 항목 제목**으로 View의 `Presentation` 확장이 조합한다(알럿 컴포넌트는 문구를 모른다) — 단일 선택은 접두어 없이 제목만("{제목}"), **다중 선택은 "Re : {제목} 외 N작품"**(사용자 확정, #269 — 단일/다중 문구 규칙이 대칭이 아니다).
  ...
  - 목록이 비면 `WSSEmptyView(type: .novelNotification)`의 "작품 둘러보기" CTA가 뜨는데, 검색 화면은 다른 Feature 모듈이라 이 화면이 직접 못 연다 — `NovelNotificationListRoute.browseNovels`(#253 리뷰에서 Route로 편입 — 처음엔 "빈 상태 CTA"라 클로저로 남겼다가, App이 실제로 push하는 화면 전환 의도인데 기본값 no-op까지 있어 `MyLibraryRoute.search`와 판정이 어긋난다는 지적으로 정정)를 올리면 실제 이동은 App이 정한다(#201부터 `MypageRootView`/`LibraryRootView` 둘 다 일반 검색 화면으로 push해 배선 완료).
