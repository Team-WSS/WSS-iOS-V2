...
- **위 6종 이후 추가된 10종(`collection_tab_select`/`_sort`/`_edit_btn`/`_private_on`/`_off`/
  ...
  걸고 수정 성공은 추적하지 않는다.
- **네비바는 플랫 `WSSNavigationBar` + `.wssCustomNavigationBar()`다**(#244, 정본 [WSSComponent](../../UI/WSSComponent/CLAUDE.md)). `CreateCollectionView`·`CollectionListView`·`CollectionMyLibrarySelectView`·`CollectionSearchNovelView` 4화면 교체(우측 완료/추가는 `trailing` 슬롯). ⚠️ **`CreateCollectionView`만 `swipeBackConfirmation: { viewModel.handle(.requestClose) }`** — 좌측 back이 `requestClose`(미저장 초안 "그만하기/계속 작성" 확인 알럿)라 스와이프로 건너뛰면 안 된다(#256부터 스와이프 pop은 막되 시도가 감지되면 back과 같은 `requestClose`가 불린다 — 변경 없으면 즉시 닫힘). 나머지 3화면은 `dismiss()`(확인 없음)라 스와이프백 허용. 참고: `CreateCollectionView` 타이틀 폰트가 `.title3`→`.title2`로 통일됐다(`WSSNavigationBar` 고정값). **`CollectionDetailView`는 `WSSNavigationBar`가 아니라 `NovelDetailView`식 커스텀 몰입형 오버레이 바로 교체했다**(#244) — 히어로 이미지 위엔 투명 바 + 흰 아이콘(`navIconColor`), 스크롤되면 흰 배경 + 검정 아이콘 + 컬렉션명 타이틀 페이드인. `ZStack` 오버레이 + 바 배경만 `.ignoresSafeArea(edges:.top)`로 상태바까지 확장(버튼은 안전영역 안, 배경 Color의 히트테스트는 `allowsHitTesting(isBarSolid)`로 표시 상태와 묶는다 — 투명일 땐 바 영역 드래그가 스크롤로 통과하고, 솔리드로 콘텐츠를 덮는 동안엔 배경이 터치를 소비한다. 상시 `false`면 바에 가려 안 보이는 그리드 셀이 바 위 탭에 반응하는 탭 관통 — 2026-09-09 발견, `NovelDetailView`와 동일 수정. 감수한 손실: 솔리드 구간엔 바 영역 드래그로 스크롤 불가 — 의도한 트레이드오프라 되돌리지 말 것) + `.wssCustomNavigationBar()`로 스와이프백. 커스텀 오버레이라 `.opacity`/색 전환이 그대로 반영된다(아래 "스크롤 반응형 네비 타이틀은 `if` 구조" 함정은 시스템 툴바 전용이라 더 이상 이 화면에 적용 안 됨). ⚠️ **스크롤 전환 애니메이션(`isScrolledFromTop`)은 사용자 선호로 제거돼 즉시 전환한다**(#244 후속, 정본 [WSSComponent](../../UI/WSSComponent/CLAUDE.md) — 되살리지 말 것).
...
- ⚠️ **컬렉션 공유 카드는 작품 수만큼 표지가 붙는다(2026-09-01)** — Kakao Developers 콘솔에 만든
  **사용자 정의 템플릿 3종**(표지 1/2/3장 전용)을 작품 수로 골라 `ShareApi.shared.shareCustom`/
  ...
  커스텀 템플릿이 필요했다. **제목 구성(`NICKNAME`/`TITLE`)은 표지 1/2/3장 세 템플릿 모두 동일하다**
  — 표지 장수만 다르고 카드 텍스트 템플릿을 따로 안 둔다.
  ...
  ⚠️ **위 우려가 2026-09-22 실기기 리포트로 실제로 재현됐다 — "작품 3개인데 표지가 하나도 안 보인다".**
  원인: 리디북스 CDN(`img.ridicdn.net`) 표지 URL이 `...xxlarge?dpi=xxhdpi#1`처럼 **URL 프래그먼트(`#1`)를
  달고 내려오는데**, 이걸 그대로 `imageScrap`에 넘기면 카카오 서버가 `image_url` 파라미터 파싱에 실패해
  (`BadParameter`, `stringToParse=-`) 그 한 슬롯만 조용히 빠지고, 슬롯 고정 특성상 **그것만으로 이미지
  컴포넌트 전체가 렌더링을 포기**했다(3장 중 2장은 정상 스크랩됐는데도 카드엔 이미지가 하나도 안
  보인 이유). 고친 방법: `scrapImage(_:)`가 `imageScrap` 호출 직전 `URLComponents.fragment = nil`로
  프래그먼트를 제거한다(프래그먼트는 서버로 전송 안 되는 순수 클라이언트 힌트라 제거해도 가리키는
  이미지는 동일). **표지 URL에 다른 예외적인 문자(공백·비-ASCII 등)가 또 다른 플랫폼 CDN에서
  발견되면, `scrapImage` 진입 전 정규화 지점이 바로 여기다** — 새 증상이 나오면 이 함수부터 볼 것.
