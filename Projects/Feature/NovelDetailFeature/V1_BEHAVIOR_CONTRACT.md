...
- ❓ **Unknown (헤드라인)** — V1은 헤더 작가 이름 탭 → **그 작가명으로 검색 결과 화면을 push**(`pushToNormalSearchViewController(searchText: authorName)`).
  ...
  - **판정 포인트**: 후속으로 작가 검색 라우팅을 배선할 때 목적지가 V1과 같은 검색 결과 화면인지 확인.
