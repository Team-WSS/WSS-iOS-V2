...
- **네비바 우측 아이콘 버튼(종·threedots)은 44×44 탭 타깃을 아이콘 프레임 뒤·`contentShape` 앞에 별도로 감싼다**(`.frame(width:24/20,height:24/20)` → `.frame(width:44,height:44)` → `.contentShape`, 뒤로가기 버튼과 동일 패턴). 이 프레임이 커지면 그만큼 우측 클러스터 전체 폭이 늘어나므로(현재 144pt: 20+44+16+44+20), 스크롤 반응형 네비 타이틀의 `.padding(.trailing,)`도 함께 갱신해야 제목이 아이콘과 안 겹친다 — 아이콘 하나 추가/크기 변경 시 이 폭 계산을 다시 할 것.
