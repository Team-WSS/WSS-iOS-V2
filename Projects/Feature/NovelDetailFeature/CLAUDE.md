...
- 진입점: `NovelDetailFeatureFactory.makeView(...)` — UseCase 10종 + `onRoute: (NovelDetailRoute) -> Void`(화면 전환 의도 7케이스, 정본은 `Sources/Navigation/NovelDetailRoute.swift`) + `onAuthenticationRequired`(#253에서 클로저 7개를 Route enum 하나로 통합 — 이 모듈이 그 패턴의 파일럿). 호출자(App)는 exhaustive switch로 자기 `Destination`에 매핑한다.
