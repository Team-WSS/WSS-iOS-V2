...
- **로드**: 첫 진입은 `LoadNovelUseCase`로 전면 스피너와 함께 `NovelInformation` 확보(`hasLoaded` 세움). **재진입(pop 복귀)은 조용한 갱신** — `onAppear`가 재진입마다 불리는 걸 이용해, `hasLoaded` 후에도 `load()`가 **스피너 없이** `loadNovel`을 다시 태워 `information`/`novel`만 제자리 교체한다(피드·페이지네이션·스크롤 보존, 실패해도 기존 화면 유지). 이유: 평가 화면(push)에서 별점·읽기상태·키워드를 고치고 돌아오면 상세의 유저 평가·독자 평가 집계가 **복귀 즉시** 최신화돼야 하기 때문(#221, iPhone SE 실측 — pop 복귀 시 재조회 발화·스크롤 유지 확인). 예전엔 `hasLoaded` 가드가 재진입 재로드를 아예 막았으나, 그 경우 평가 수정이 반영되지 않는 stale 갭이 있었다. 피드 목록은 **피드 탭 첫 진입 시 지연 로드**, 이후 `lastFeedID` 커서 페이지네이션(첫 페이지 커서 0).
