...
- 시나리오는 `userReviewParts`(`Set<UserReviewPart>?`) / `readerReviewParts`(`Set<ReaderReviewPart>`) / `hasFeeds` **축**으로 표현하고 Mock UseCase 둘이 그 축만 읽는다 — 새 조건을 넣을 땐 축(또는 집합의 원소)을 늘리지, `if scenario == ...` 분기를 Mock 곳곳에 흩뿌리지 말 것.
