...
- ⚠️ **`genreSelectionSlot`의 `if let genreSelectionViewModel { GenreSelectionView(viewModel: ...) }`는 `.id(ObjectIdentifier(genreSelectionViewModel))`가 없으면 재확정이 실제로 반영되지 않는다.** 성별/출생년도를 뒤로 가서 다시 확정하면 `handleGenderBirthYearConfirmed`가 **새 `GenreSelectionViewModel` 인스턴스**를 만들어 대입하지만, `if let`의 분기 자체(true)는 안 바뀌어 `GenreSelectionView`의 뷰 정체성이 유지된다 — SwiftUI `@State`는 그 정체성의 **최초 initialValue만 쓰고 이후 새로 넘어온 인스턴스를 무시**한다(`Feature/CLAUDE.md`의 `.sheet(isPresented:)` 트랩과 동일 메커니즘). 결과적으로 화면엔 옛 인스턴스가 계속 렌더되고, 그 인스턴스의 `gender`/`birthYear`는 `let`이라 갱신될 방법이 없어 **최종 프로필 등록에 재확정 이전 값이 그대로 나간다**(실측, #178 리뷰에서 발견). `.id()`로 인스턴스가 바뀔 때마다 뷰 정체성도 강제로 갈아 `@State`가 다시 시드되게 해야 한다.
