...
- ⚠️ **인트로 에러 토스트는 공용 `.showWSSToast`를 안 쓰고 body에 직접 배치한다(#257).** 공용 `WSSToastViewModifier`는 토스트를 content 크기 ZStack에 붙이는데, 인트로는 고정 배너(567)로 **작은 기기(SE)에서 content가 화면 아래로 넘쳐** 토스트가 그 넘친 바닥을 따라가 잘렸다(실측). 그래서 body를 `GeometryReader { }.ignoresSafeArea()`로 감싸 윈도우 크기를 잡고, ① `backgroundLayer`(배경 이미지+흰색)를 full-bleed로 깔고, ② `foregroundContent`(배너·버튼)는 `.padding(.top/.bottom, proxy.safeAreaInsets)`로 세이프에어리어에 배치하고, ③ 토스트를 윈도우 프레임에 `.padding(.bottom, 40 + safeAreaInsets.bottom)`로 고정한다. **배경을 전경과 분리하지 않으면**(전경만 패딩) 인셋 영역에 흰 띠가 생기고, **bottom 인셋을 복원 안 하면** 노치 기기에서 버튼이 홈 인디케이터 쪽으로 내려간다(둘 다 리뷰 지적으로 수정, SE는 하단 인셋 0이라 안 잡힘 — 노치 기기로도 확인할 것). 자동 닫힘 타이머(1.5초)도 `.task(id: hasLoginError)`로 이 화면에 내장했다(공용 모디파이어의 `.task(id: isPresented)`와 동일 패턴). 같은 full-bleed 오버플로 화면이 또 나오면 이 배치를 WSSComponent로 승격할지 검토(현재는 이 화면 전용 스코프).
