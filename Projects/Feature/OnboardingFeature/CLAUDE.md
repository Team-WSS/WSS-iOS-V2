...
- **`OnboardingIntroView.content`의 배너↔`bottomSection` 사이 `Spacer()`(유연)는 지우면 안 된다** — 하단 요소(도트+소셜 버튼)를 화면 아래로 밀어붙이는 유일한 메커니즘이라, 이걸 없애고 고정 `Spacer().frame(height:)`로 대체하려 하면(비로그인 버튼 제거 때 실제로 그렇게 했다가 리뷰에서 걸림) `TabView` 배너의 암묵적 사이징과 얽혀 기기별로 상·하단 여백이 들쭉날쭉해진다(실측: 의도보다 훨씬 크고 불안정한 여백). `bottomSection` 내부 요소 개수가 바뀌어도 이 유연 `Spacer()` + 바깥 `.padding(.bottom, 24)` 조합으로 하단 고정을 유지할 것.
