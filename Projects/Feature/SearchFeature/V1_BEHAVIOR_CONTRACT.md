...
5. 🔧 **복원 확정→TODO 12절** (2026-08-28: V2 자동 포커스 없음 실측) — **검색창 자동 포커스(키보드 자동 노출) 미구현.** V1은 진입 시 `becomeFirstResponder()`로 키보드를 바로 띄운다. V2 `NormalSearchView`는 `onAppear`에서 포커스를 세우지 않는다. → [2.1](#21-진입생명주기)
6. 🔧 **복원 확정→TODO 12절** (2026-08-28: V2 제한 없음 실측) — **검색창 30자 입력 제한 복원.** V1 검색 텍스트필드는 30자 초과 입력을 막는다. V2 `WSSSearchBar`엔 제한이 없다(실측). → [2.1](#21-진입생명주기)
7. 🔧 **횡단 이슈→TODO 12절** (Amplitude 재도입) — **Amplitude 애널리틱스 이벤트 전면 부재.** V1은 검색 진입·일반검색·소소픽·결과 클릭·문의 등 다수 이벤트를 track한다. V2엔 대응 이벤트 전송이 없다(Logger는 에러 로깅용). → [5.2](#52-애널리틱스)
