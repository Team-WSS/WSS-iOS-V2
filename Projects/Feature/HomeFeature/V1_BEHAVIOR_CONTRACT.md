...
- ❓ **Unknown (헤드라인)** — V1은 오늘의 인기작·지금 뜨는 글을 **Splash 단계에서 `HomePrefetchService`로 미리 받아** 두고, 홈 진입 시 `consumeTodayPopular()`/`consumeRealtimeFeeds()`로 있으면 그걸 쓰고 없으면 네트워크를 탔다(첫 홈 표시 지연 감소).
  - **V2: 이 프리페치가 없다.** 홈 진입 시 항상 새로 로드한다.
  - 근거: V1 `HomeViewModel.swift:304-311`(`HomePrefetchService.shared.consume…`) · V2 `LoadHomeDataUseCase.swift`(프리페치 경로 없음)
  - **판정 포인트**: Splash 프리페치로 첫 홈을 빠르게 띄우던 최적화를 되살릴지 / 진입 시 로드가 의도인지.
...
- ❓ **Unknown (헤드라인)** — V1 홈은 `APIConstants.isLogined`로 **거의 모든 동작이 갈렸다**:
  ...
  - **판정 포인트**: V2가 **로그인 필수 앱**(비로그인 홈이 존재하지 않음)이라 이 경로가 통째로 불필요한 것인지 / 게스트 홈을 어딘가에서 되살려야 하는지. (V2 아키텍처상 전자로 보이나 명문 근거 미확인.)
...
- ❓ **Unknown (헤드라인)** — **부분 실패 처리**. V1은 오늘의 인기작·지금 뜨는 글·취향추천·알림 조회를 **각각 `.catch`로 빈 값 폴백**한다 → 한 섹션이 실패해도 **그 섹션만 비고 나머지 홈은 정상 표시**되며, **전면 에러 화면이 없다**(바깥 `onError`는 스피너만 끈다).
  ...
  - **판정 포인트**: "한 섹션 실패 시 나머지는 그대로 보여주던" **그레이스풀 저하를 버리고 전면 실패로 일원화**한 게 의도인지. (전면 실패 뷰=의도는 맞으나, *부분 성공을 아예 안 보여주는* 쪽 효과는 별도 확인.)
...
- ❓ **Unknown (헤드라인)** — **필수 약관 동의 강제**. V1은 홈 첫 진입(`viewDidLoad` → `getUserMeData` 성공 후)에서 `getTermSetting`을 조회해 **필수 약관 미동의면 동의 알럿**을 띄우고 약관 동의 모달을 present했다.
  ...
  - **판정 포인트**: 약관 동의 강제가 **다른 화면으로 이동**했는지 / 누락인지.
- ❓ **Unknown (헤드라인)** — **앱 최소 버전 강제 업데이트**. V1은 `viewWillAppear`마다 `getAppMinimumVersion`을 조회해 현재 버전이 낮으면 **닫을 수 없는(`isDismissable: false`) 업데이트 알럿** → App Store로 보냈다.
  ...
  - **판정 포인트**: 강제 업데이트 게이트가 앱 어딘가에 있는지 / 홈에서 빠지며 사라졌는지.
