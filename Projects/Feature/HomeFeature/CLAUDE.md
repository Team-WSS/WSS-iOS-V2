...
- **로드**: `onAppear`마다 `.load` → `LoadHomeDataUseCase`(추천 3종)와 `LoadUnreadNotificationStatusUseCase`
  ...
  - **수확은 콘텐츠·배지 따로다(2026-08-31)** — 콘텐츠(추천 3종)는 성공 즉시 그리고, 배지는 도착하는 대로
    따로 반영한다. 한 do/catch로 묶으면 프리페치로 즉시 끝나는 콘텐츠가 배지 네트워크 왕복까지 기다렸다가
    그려져 **첫 런치의 프리페치 이득이 죽는다**. 되묶지 말 것.
  - **실패도 분리다**: 추천 3종 실패만 전면 실패 뷰("하나라도 실패하면 홈 전체 실패"는 이제 콘텐츠 3종
    한정). **배지 실패는 조용히 무시(로그만, 사용자 확정 2026-08-31)** — #195의 부수 데이터 토스트 lane도
    일부러 안 쓴다: 배지는 실패해도 "보이는 고장"이 없어 멀쩡한 홈 위의 에러 토스트가 설명이 안 되고,
    탭 복귀 갱신마다 반복 노출만 된다(다음 탭 복귀가 자동 재시도 = self-healing). 인증 만료만은 배지
    경로에서도 로그인 라우팅을 태운다(idempotent 계약이라 안전).
...
- **섹션이 0건이면 제목까지 통째로 숨긴다**(빈 문구 ❌). 추천 콘텐츠라 "없음"을 굳이 말할 이유가 없다.
  구 WSSiOS도 섹션별 `isHidden`으로 처리했다. **단 "이 웹소설은 어때요?"(선호장르) 섹션은 예외** —
  미설정(`.noGenreSettings`)뿐 아니라 **설정했으나 추천 0건(`.novels([])`)일 때도** 숨기지 않고 같은 **설정
  유도 카드**를 띄운다(#222 V1 parity — 빈 자리보다 행동 유도가 낫다, 사용자 확정). 분기는
  `PreferenceGenreSection`이 상태로 판단하고, `HomeView`는 `state` != nil이면 무조건 섹션을 그린다(nil=로드 전만
  숨김). 예전엔 `isEmptyNovels` 가드로 0건을 숨겼으나 걷어냈다 — 되살리지 말 것. (오늘의 발견·추천글은
  여전히 0건이면 숨긴다.)
...
- **`home` 화면진입 이벤트는 `load()`의 `!hasLoadedContent` 분기에서만 쏜다**(#250 리뷰로 정정) — 위
  "`.load`에 최초 1회 가드를 넣지 말 것" 규칙은 **재조회**에만 적용된다. 트래킹 호출은 그 재조회 Task
  스폰 앞에 별도로 `if !hasLoadedContent`를 걸어, 탭 복귀마다 갱신은 그대로 두면서 스크린뷰 집계만
  최초 1회로 좁혔다 — 이 둘을 같은 가드로 착각해 트래킹까지 매번 걸리게 되돌리지 말 것.
