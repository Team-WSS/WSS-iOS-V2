<!-- 모듈 가이드. 이 모듈 작업 시 상위 Projects/Feature/AGENTS.md(레이어 규칙·State/Action 골격)와 함께 자동 로드됨. -->
# NovelReviewFeature
...
- **뒤로가기 요청은 View가 직접 `hasUnsavedChanges`/`dismiss()`를 판단하지 않고 `requestClose` 액션으로만 보낸다.** 로드 중이면 즉시 `shouldDismiss = true`로 닫고, `loadTask.cancel()` + `isClosing` guard로 뒤늦게 도착한 로드 결과가 `draft`/`isLoading`/`presentedError`를 갱신하지 못하게 유지한다.
