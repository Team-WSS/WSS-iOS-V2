...
> **판정 상태(2026-08-28)**: 전부 판정 완료. **판정 대기·보류 없음**(전부 개발 단독 결정). 결정된 것은 `docs/TODO.md` 12절, 나머지는 의도적 개선(확인 완료). 본문 각 절 배지가 정본.
...
5. 🔧 **POST/PUT 서버 주도** — V1 클라이언트 추정 → V2 항상 POST 후 `USER_NOVEL-002`면 PUT 폴백. → [2.3](#23-생성post-vs-수정put-결정)
...
8. 🗑 **Amplitude 이벤트 제거** — 저장 `rateNovel`·키워드 `contactKeyword`. V2 없음(홈·피드와 함께 횡단 재도입 대상, 12절). → [2.4](#24-저장-부수-작업앱-리뷰-요청노티피케이션분석)
