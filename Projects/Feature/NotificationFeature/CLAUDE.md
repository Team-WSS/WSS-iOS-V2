...
- **목록 로드**: 첫 진입은 로딩 뷰와 함께 첫 페이지(`hasLoaded`는 성공 시만 소진), **재진입(push 복귀)은
  스피너 없이 첫 페이지를 조용히 재조회해 목록을 교체한다**(#236, V1 parity 복원 — 복귀 사이 서버에서
  바뀐 새 알림·다른 기기 읽음 반영. 실패하면 기존 화면 유지, 성공 시 페이지네이션·`markedAsReadIDs` 리셋).
  - **깊이 스크롤 후 복귀 시 목록이 첫 페이지로 줄어드는 건 판정된 절충이다**(#236 리뷰에서 인지) —
    알림은 최신이 맨 위라 복귀 갱신의 목적이 "새 알림 반영"이고 깊은 위치 보존의 가치가 작다.
    NovelDetail 피드처럼 `size = 보던 개수`로 보존하려면 가능은 하나(UseCase가 이미 size를 받음)
    화면 성격상 채택하지 않았다 — "일관되게 맞추자"며 보존을 얹으려면 이 판정부터 뒤집을 것.
  → 커서 무한 스크롤. 커서는 서버 발급 문자열이 아니라
...
- **셀 탭 = 두 가지 일**: ① VM이 읽음 처리 — 목록 표시는 **항상** 낙관 반영하되 `MarkNotificationAsReadUseCase`
  ...
  발화(`.notificationDetail`→알림 상세, `.feedDetail`→피드, `.novelDetail`→작품 상세, `.unknown`/nil→전환 없음).
...
- ⚠️ **상세로 가는 알림은 읽음 처리가 상세 조회 성공에 종속된다** — `read`를 안 보내므로, 상세 GET이
  네트워크 실패로 끝나면 **목록만 읽음이고 서버는 미읽음**인 채로 남는다(재진입하면 미읽음으로 되돌아온다).
  아래 "롤백하지 않는다"와 같은 방향의 감수지만, 이쪽은 서버 반영 자체가 안 일어난다는 점에서 더 취약하다.
  실제로 문제가 되면 상세 로드 실패 시 `read`를 보내는 보완을 넣을 수 있다(지금은 일부러 없다).
...
- ⚠️ **목록 로드에 취소·무효화 장치가 일부러 없다** — `reloadFromScratch`가 `loadTask?.cancel()`도, 세대(generation)
  카운터도 쓰지 않는다. 모든 호출자(`load`·`retry`·재진입 조용한 재조회 #236)가 `loadTask == nil`을 선행 확인해
  **인플라이트 요청이 있는 채로 재로드가 걸리는 경로 자체가 없기** 때문이다(재시도 버튼은 전면 실패 뷰에서만
  보이고 그때 목록은 비어 있다). 재진입 재조회도 이 직렬화 위에 있어 취소 없이 안전하다 — 단 인플라이트가
  있으면 그 회차 재조회는 조용히 스킵된다(다음 재진입이 다시 시도).
  - **재로드 경로를 하나라도 추가하면**(당겨서 새로고침·"전체 읽음"·필터 등) **그날 깨진다** — 늦게 도착한 옛 응답이
    새 목록을 덮는다. 그때는 `cancel()`을 넣는 동시에 **`loadPage`의 `defer` 정리를 `if !Task.isCancelled`로
    감싸야** 한다(Swift의 취소는 협력적이라 `await` 재개 뒤엔 `catch`·`defer`가 그대로 돈다 — 취소된 로드가
