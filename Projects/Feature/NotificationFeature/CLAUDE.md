...
- ⚠️ **목록 로드에 취소·무효화 장치가 일부러 없다** — `reloadFromScratch`가 `loadTask?.cancel()`도, 세대(generation)
  ...
  - **재로드 경로를 하나라도 추가하면**(당겨서 새로고침·"전체 읽음"·필터 등) **그날 깨진다** — 늦게 도착한 옛 응답이
    새 목록을 덮는다. 그때는 `cancel()`만 넣지 말고 **무효화까지 같이** 넣어야 한다(Swift의 취소는 협력적이라
    `await` 재개 뒤엔 `catch`·`defer`가 그대로 돈다). 서재(`LibraryViewModel`)가 필터·정렬 때문에 그 경로를 갖고 있어
    세대 카운터를 쓰는 정본이다. → [LibraryFeature](../LibraryFeature/CLAUDE.md)
  - #181 리뷰에서 세대 카운터를 한 번 넣었다가 **지금 쓰이지 않는 배관이라 되돌렸다** — "정본이 쓰니까"를 근거로
    다시 넣지 말 것. 넣을 근거는 정본이 아니라 **이 화면에 재로드 경로가 생겼다는 사실**이다.
