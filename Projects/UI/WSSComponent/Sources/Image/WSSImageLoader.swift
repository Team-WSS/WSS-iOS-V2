...
/// `Image` 값 자체가 필요한 자리는 뷰로는 못 받기 때문. 그 시트는 폐기돼 지금 호출자는 `WSSAsyncImage`뿐이라
/// `internal`로 둔다(공개 표면 최소화 — 모듈 밖에서 필요해지는 시점에 `public`으로 연다). 값으로 받은 URL에서
/// 바이트만 읽을 뿐 앱 네트워킹 스택·인증은 모른다(`UI/CLAUDE.md`의 "URL 기반 이미지 로딩은 표현 인프라" 예외 그대로).
enum WSSImageLoader {
