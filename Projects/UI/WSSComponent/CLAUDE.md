...
- **`WSSNavigationBar`는 커스텀 헤더 화면의 공용 상단 바다**(#181에 승격, #244에 전 화면 표준으로 확대). 시스템 네비바를 안 쓰는 이유는 폰트·back 아이콘이 디자인과 달라서고(iOS 26에선 시스템 바가 리퀴드 글래스로도 뜬다), **플랫하게 통일**하려는 목적이다.
  ...
  - ⚠️ **`swipeBackEnabled: true`(기본)일 땐 `.navigationBarBackButtonHidden`을 아예 걸지 않는다(기본값 유지) — `false`로도 명시하지 말 것.** iOS 26에선 `.navigationBarBackButtonHidden(false)`를 명시로 걸면 `.toolbar(.hidden)`으로 바를 숨겨도 **시스템 back 버튼(리퀴드 글래스)이 강제로 떠서**, 불투명 커스텀 바 화면에선 그 뒤에 숨고 **투명 바 화면(컬렉션 상세 등)에선 히어로 위로 비쳐 보인다**(#244 사용자 리포트로 발견). 그래서 modifier는 허용 시 `.toolbar(.hidden)` + `.enableSwipeBack()`만 걸고(Notification·NovelDetail 검증 패턴), 안 건 기본값(hidesBackButton=false)이 스와이프 delegate 가드도 통과시킨다.
