...
### 8. 피드 더보기 실패에 재시도 UI (#195에서 남김)

`NovelDetail` 피드 탭은 **더보기 실패만 토스트**로 남아 있다 — 목록이 이미 있어 실패를 알릴 자리가
화면에 없기 때문이다(이 탭엔 재시도 버튼이 없고 빈 상태 문구뿐). 서재는 같은 상황에서 목록을 걷어내고
재시도 버튼이 달린 `NetworkErrorView`로 대체해 규칙을 통일했다.

- **왜 미뤘나**: 목록을 걷어내면 복구 수단이 "탭 재탭"이라는 숨은 동작뿐이라 서재보다 나빠진다.
  재시도 UI(하단 재시도 행이든 실패 뷰든)를 **디자인과 함께 정해야** 한다.
- **왜 중요한가**: 토스트는 사라지면 다시 부를 방법이 없다. 서재에서 실제로 "하단에서 더보기가 실패하면
  그 뒤로 목록이 영영 안 채워지는" 상태를 만들었다(#195 실측).
- 규칙 정본: `Projects/Feature/CLAUDE.md`의 "로드 실패 표현 계약".
