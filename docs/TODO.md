...
- **어디를 고치나(남은 것, 컷오버 시점에)**:
  ...
  8. **`CUTOVER_READY` GitHub Actions repo variable을 `true`로 전환** — `main` push 시 App Store 심사
     ...
     **✅ 이미 끝난 것(2026-09-13)**: `app-store-release` Environment 생성 + Required reviewers(Guryss·
     ...
     ✅ **GitHub Actions secrets 17/17 전부 등록 완료(2026-09-21)** — `app-store-release` environment
     secrets로(레포 시크릿이 아니라 **environment 시크릿** — 그 environment를 쓰는 job에서만 보이게
     최소 권한 유지, `gh secret set NAME --env app-store-release`). 로컬 심사 제출은 macOS Keychain의
     match 캐시·로컬 `Config/*.xcconfig`로 됐던 것이라 CI(GitHub Actions 러너, 매번 새로 뜨는 빈 macOS
     VM)는 이 값들이 전혀 없어서 등록이 필요했다.
       - `ASC_KEY_ID`·`ASC_ISSUER_ID`·`ASC_KEY_P8_BASE64`·`TEAM_ID`·`APP_IDENTIFIER_RELEASE`·
         `APP_IDENTIFIER_DEBUG` — `fastlane/.env` + `fastlane/*.p8`에서 추출.
       ...
       - `MATCH_GIT_DEPLOY_KEY` — `WSS-iOS-Certificates` 레포 전용 신규 읽기전용 SSH 키쌍을 만들어
         공개키는 그 레포 Deploy keys에, 개인키는 이 시크릿 값으로 등록(**한 번만 등록하면 되는 레포
         전역 설정** — 팀원 각자 새로 만들 필요 없음).
       - `MATCH_PASSWORD` — 팀 채널/1Password의 기존 값을 그대로 등록.
     **팀원도 이 등록 작업을 할 수 있나**: ✅ 권한은 이미 충분하다(2026-09-21 확인) — `WSS-iOS-V2`·
...
- **피드 탭 목록(`SosoFeedView`) 인증 만료 로그인 라우팅 배관** — #256에서 목록 로드 실패 표현(#195,
  `NetworkErrorView`+재시도)은 들어왔지만 `onAuthenticationRequired` 배관은 여전히 없어, 401이 "일시적
  오류" 실패 뷰로 흡수되고 재시도는 같은 401로 되돌아온다(예전 조용한 빈 화면보단 낫지만 계약 위반은
  그대로). 위 UserPage 항목과 같은 배선 작업. → `FeedFeature`/`App(FeedRootView)`.
...
- **보류 이유**: ① 실익이 좁고(피드 탭·작품 상세는 이미 셀 동기화로 자기 화면 내 왕복을 커버), ② Domain
  엔티티+테스트·4개 VM 주입·App 배선이 걸리는 횡단 변경이라 출시(9/8경) 직전 리스크 대비 이득이 작다.
  - *(2026-09-08 전제 갱신, #256)* 처음 보류할 땐 "작품 상세는 재진입 전체 재조회가 셀 동기화의 상위호환"
    이 근거였으나, 그 전체 재조회(size=보던 개수 window 교체)는 **새 글이 생기면 기존 글이 창 밖으로 밀려
    사라져 보이는 실결함**으로 판명돼 #256에서 셀 동기화+작성 복귀 리셋으로 교체됐다(재판정 기록은
    `NovelDetailFeature/CLAUDE.md` 로드 절). 상위호환 전제는 폐기 — 대신 "타 유저 변경의 재진입 반영 없음"
    절충이 작품 상세·유저 프로필 계열로도 넓어졌으니, 이 변경 로그를 되살리면 실익 범위도 그만큼 커졌다.
- **⚠️ 되살릴 때 함정(설계 논의에서 확정)**: 작품 상세 피드 탭·전체 피드 목록엔 **당겨서 새로고침이 없다** —
  셀 동기화만으로는 다른 유저의 새 글·변경이 화면을 새로 push하기 전까지 영영 안 들어온다(#256이 이
  절충을 판정하고 감수했다 — 해소하려면 pull-to-refresh 추가가 선행). 작성 완료는 FeedID가 없어(생성
...
- **무엇**: #253이 화면 전환 클로저의 기본값 no-op을 전부 없앴지만(Route enum + 필수 `onRoute`),
  Route 비대상으로 남긴 세션 이벤트·완료 결과 콜백에는 기본값이 남아 있다 —
  `onLogoutSuccess`/`onWithdrawSuccess`/`onSaveSuccess`(SettingFeatureFactory)·일부 `onSubmitted`류.
  "기본값 no-op이 배선 누락을 조용히 삼킨다"는 위험 자체는 동일하다(예: 새 조립 지점이
  `onLogoutSuccess`를 빠뜨리면 로그아웃해도 세션이 안 끝나는데 컴파일이 통과).
- **왜 미룸**: #253의 계약이 "동작 불변 배선 리팩토링"이라 시그니처 의미 변경(기본값 제거 = 전 호출부
